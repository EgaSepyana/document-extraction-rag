...
async def upload(file: UploadFile = File(...), dto: DocumentDTO = Depends()):
    start_time = time()
    try:
        return BaseResponse(data=obj.upload(file, dto),
...
async def index(dto: IndexingRequestDTO):
    start_time = time()
    try:
        return BaseResponse(data=obj.index(dto),
                            metaData=MetadataSuccess(execution_time=get_execution_time(start_time)))
    except Exception as error:
        traceback.print_exc()
        raise HTTPException(status_code=400, detail=BaseResponseFailed(
...
async def chat(dto: ChatRequestDTO):
    start_time = time()
    try:
        return BaseResponse(data=obj.chat(dto),
                            metaData=MetadataSuccess(execution_time=get_execution_time(start_time)))
    except Exception as error:
        traceback.print_exc()
        raise HTTPException(status_code=400, detail=BaseResponseFailed(
...
async def chat(id: str):
    start_time = time()
    try:
        return BaseResponse(data=obj.docs(id),
                            metaData=MetadataSuccess(execution_time=get_execution_time(start_time)))
    except Exception as error:
        traceback.print_exc()
        raise HTTPException(status_code=400, detail=BaseResponseFailed(
